...
salary - lag(salary) over(partition by department order by hire_date) from employees;
...
-- Q9) Total sales per row (same total repeated using windowSUM)
select *, sum(amount) over() from employees;
...
-- and than we access  the column and we  filter out of the data
